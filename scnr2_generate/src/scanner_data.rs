...
use crate::{
    pattern::{
        Pattern, STATE_CAP_PLACEHOLDER, STATE_SEG_PLACEHOLDER, StateOpSegment,
...
fn replace_placeholder_once(
    ...
    match (first, second) {
        (None, _) => Err(syn::Error::new(
            proc_macro2::Span::call_site(),
            format!("internal error: missing state placeholder in {context} during resolution"),
        )),
        (Some(_), Some(_)) => Err(syn::Error::new(
            proc_macro2::Span::call_site(),
            format!("internal error: multiple state placeholders in {context} during resolution"),
        )),
        (Some((idx, _)), None) => {
...
fn parse_state_declaration(content: &syn::parse::ParseBuffer) -> syn::Result<StateDeclaration> {
    if !cfg!(feature = "dynamic-state") {
        return Err(
            content.error("state declarations require the `dynamic-state` feature on scnr2")
        );
    }
            ...
            let max = if is_inclusive {
                if min > max {
                    return Err(paren_content
                        .error("state range must satisfy min <= max for inclusive ranges"));
                }
                max
            } else {
                if min >= max {
                    return Err(paren_content
                        .error("state range must satisfy min < max for exclusive ranges"));
                }
                max - 1
            };
...
mod tests {
    ...
    use crate::pattern::{CompileTimeConstraint, CompileTimeExpr, StateOpSegment};
    ...
    #[test]
    fn test_state_declarations_require_feature() {
        let input = quote::quote! {
            FeatureGatedScanner {
                state n: count(0..=4);
                mode INITIAL {
        ...
        let err = syn::parse2::<ScannerData>(input).unwrap_err();
        assert!(
            err.to_string()
                .contains("state declarations require the `dynamic-state` feature")
        );
    }
}
